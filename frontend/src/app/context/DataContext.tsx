...
interface DataContextType {
  ...
  // Client
  addClient: (data: { id_cliente: string; nombre: string; nit?: string; cedula?: string; tipo_cliente?: string; email?: string; nivel_partner?: string }) => Promise<boolean>;
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const updateLotStatus = useCallback(async (id: string, estado: string): Promise<boolean> => {
    try {
      ...
      await fetchProductionLots();
      await fetchProducts();   // Stock se actualiza al completar un lote
      return true;
    } catch (err: any) {
      setError(err.message);
      return false;
  ...
  // Clients
  const addClient = useCallback(async (data: { id_cliente: string; nombre: string; nit?: string; cedula?: string; tipo_cliente?: string; email?: string; nivel_partner?: string }): Promise<boolean> => {
    try {
