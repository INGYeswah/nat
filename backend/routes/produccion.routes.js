...
router.patch('/lotes/:id/status', auth, permiso, async (req, res) => {
  ...
  try {
    // Obtener datos del lote antes de actualizar
    const [lotRows] = await conn.query(
      'SELECT id_producto, cantidad_esperada, estado_lote FROM LOTE_PRODUCCION WHERE id_lote=?',
      [req.params.id]
    );
    if (!lotRows.length) {
      await conn.rollback();
    ...
    const lote = lotRows[0];

    // Solo permitir transiciones válidas
    // En Proceso → Completado o Cancelado
    // No permitir re-cambiar desde Completado/Cancelado
    if (lote.estado_lote === 'Completado' || lote.estado_lote === 'Cancelado') {
      await conn.rollback();
      return res.status(400).json({ error: `No se puede cambiar el estado de un lote '${lote.estado_lote}'.` });
    }
    ...
    // Si el lote se completa, agregar la cantidad producida al stock del producto
    if (estado === 'Completado' && lote.estado_lote === 'En Proceso') {
      await conn.query(
        'UPDATE PRODUCTO SET stock_actual = stock_actual + ? WHERE id_producto = ?',
        [lote.cantidad_esperada, lote.id_producto]
      );
    }

    await conn.commit();
    res.json({ mensaje: `Estado del lote actualizado a '${estado}'.${estado === 'Completado' ? ' Stock del producto actualizado.' : ''}` });
  } catch (err) {
    await conn.rollback();
    console.error(err);
